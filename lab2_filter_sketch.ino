...
bool alarmState = false; // Add flag for if sensor is activated
...
void loop()
{
   ...
   while (1)
   {
      ...
      if ((micros() - changet) > 1000e3)
      {
         ...
         // When Beam is Interruption
         if (maxs < threshold_val)
         {
            digitalWrite(LED, HIGH);
            if (alarmState = false)
               ;
            Serial.print("ALERT: SENSOR_INTERRUPTED");
            // SEND UPDATE TO COMPUTER --> SEND EMAIL
            alarmState = true;
         }
         // No Interruption
         else
